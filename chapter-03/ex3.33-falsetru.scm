...
(define (adder a1 a2 sum)
  ...
  (define (me request)
    (cond ((eq? request 'I-have-a-value)  (process-new-value))
          ...
          (else                           (erorr "Unknown request -- ADDER" request))))
  (connect a1 me)
  (connect a2 me)
  (connect sum me)
  me)
...
(define (make-connector)
  (let ((value false) (informant false) (constraints '()))
    ...
    (define (connect new-constraint)
      (if (not (memq new-constraint constraints)) (set! constraints (cons new-constraint constraints)))
    ...
    (define (me request)
      (cond ((eq? request 'has-value?) (if informant true false))
            ...
            (else (erorr "Unknown operation -- CONNECTOR" request))))
    me))
...
(define (averager a1 a2 avg)
  (define (process-new-value)
    (cond ((and (has-value? a1) (has-value? a2))
           (set-value! avg (/ (+ (get-value a1) (get-value a2)) 2) me))
          ((and (has-value? a1) (has-value? avg))
           (set-value! a2 (- (* (get-value avg) 2) (get-value a1)) me))
          ((and (has-value? a2) (has-value? avg))
           (set-value! a1 (- (* (get-value avg) 2) (get-value a2)) me))))
  (define (process-forget-value)
    (forget-value! avg me)
    (forget-value! a1 me)
    (forget-value! a2 me)
  ...
  (define (me request)
    (cond ((eq? request 'I-have-a-value)  (process-new-value))
          ((eq? request 'I-lost-my-value) (process-forget-value))
          (else                           (erorr "Unknown request -- AVERAGER" request))))
  (connect a1 me)
  (connect a2 me)
  (connect avg me)
  me)
