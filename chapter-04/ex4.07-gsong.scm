...
; 4.07 solution
(define (make-let decllist body)
  (list 'let decllist body))
(define (make-let* decllist body)
  (list 'let* decllist body))
(define (let*->nested-lets exp)
  (if (null? (cadr exp)) (caddr exp)
  (let ((firstdecl (caadr exp))
        (restdecl (cdadr exp))
        (body (caddr exp)))
    '(if (null? firstdecl) body
        (make-let (list firstdecl) (make-let* restdecl body))))))
...
(run (make-testcase 
     ...
     ; '(assert-equal? 2 (myeval '(cond ((assoc b ((a 1) (b 2))) => cadr) (else false)) the-global-environment))
      ...
      '(assert-equal? 39 (myeval '(let* ((x 3) (y (+ x 2)) (z (+ x y 5))) (* x z)) the-global-environment))
      ))