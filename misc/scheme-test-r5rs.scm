...
(define (named-result-log name result)
  (display "**** [")
  (display name)
  (display "] Test finished (Pass  : ") (display (pass-count result))
  (display ", Fail  : ") (display (fail-count result))
  (display ", Error : ") (display (error-count result))
  (display ")\n"))
...
(define (eval-test test)
  (cond 
    ((equal? (assert-exp? test) #f)
     ...
     result-error)
    ((equal? (car (evaluate test)) #t)
     result-pass)
    (else
     (fail-log test (cdr (evaluate test)))
     result-fail)))
...
(define (run-named-test name test)
  (named-result-log name (run-one-step test 0 0 0)))